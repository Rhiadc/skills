# Architecture & DDD lens (Go) — development

Structure and domain clarity for the **change you are building**. Prefer the smallest structure that keeps boundaries sound. Do not demand full DDD ceremony on tiny CLIs or pure adapters.
...
| Avoid producer-side interfaces | Do not put huge `Repository` iface beside Postgres with one consumer |
...
| `any` says nothing (#8) | Prefer real types/constraints |

---

## DDD mapping in Go

| DDD concept | Go shape | Build checks |
|-------------|----------|--------------|
...
**When NOT to push DDD:** trivial CRUD, generated clients, one-off scripts — keep flat and clear rather than over-engineering.
...
Layout patterns (any is fine if consistent with the repo):
...
Avoid **churn** that spreads across many unrelated packages without a facade — introduce an aggregate/ACL when the change keeps fan-out growing.
...
## When documenting an architecture gap
