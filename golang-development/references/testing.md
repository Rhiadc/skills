# Tests lens (Go) — development

Own **test design as you implement**, coverage of the change, hermeticity, and CI-relevant flags. Production concurrency bugs stay with the concurrency lens; you add `-race` / no-sleep-sync / leak tests here.
...
1. Behavior change → write a test that fails if reverted
...
## Must-cover the change (#85 spirit + Cheney)
...
**Severity if skipped:** production behavior change with no test for the new path → **medium** (or **high** if authz/money/data integrity). Style-only table-driven polish → **nit**.
...
- [ ] Packages that start goroutines: run under `go test -race` (or note CI gap) (#83)
...
- **Benchmarks (#89):** only when claiming performance or touching hot path; require `b.N` discipline, `ReportAllocs` when allocs matter; avoid dead-code elimination / wrong timer placement
